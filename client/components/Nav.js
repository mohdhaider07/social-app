                    ...
                    <Link href='register'>
                        ...
                        <div className='flex'>
                           ...
                           { state&&state.user&&state.token&&<Link href='/user/dashboard'><a className={`mx-2 ${current==="/user/dashboard"&&"border-b-2 border-cyan-500"}`}>{state.user.name}</a></Link>}
                            ...
                            <div>
                           { state&&state.user&&state.token&&state.user.role==="Admin"&&<Link href='/admin'><a className={`mx-2 ${current==="/admin"&&"border-b-2 border-cyan-500"}`}>Admin</a></Link>}
                            </div>
                            <div>
                           { state&&state.user&&state.token&&<Link href='/user/profile/update'><a className={`mx-2 ${current==="/user/profile/update"&&"border-b-2 border-cyan-500"}`}>Profile</a></Link>}
                            </div>
